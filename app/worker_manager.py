...
def save_error_case(
    sub: Submission,
    result: ProcessExecuteResult | None = None,
...
def executor_factory(
    type: str, timeout: int, memory_limit: int, cpu_core: int
) -> ScriptExecutor:
    if type == "python":
...
class Worker(Process):
    ...
    def _run_loop(self):
        ...
        if abs(time_offset) > 1:
            logger.warning(
                f"Clock skew detected: {time_offset:.2f} seconds. "
        ...
        while True:
            ...
            work_item = redis_queue.pqueue.block_pop(
                app_config.REDIS_WORK_QUEUE_NAME,
                timeout=app_config.REDIS_WORK_QUEUE_BLOCK_TIMEOUT,
            )
            if not work_item:
                continue
            ...
            try:
                ...
                if (
                    not long_running
                    and (lifetime := time() - payload.timestamp)
                    >= app_config.MAX_QUEUE_WORK_LIFE_TIME
                ):
                    logger.warning(
                        f"Work {payload.work_id} lifetime ({lifetime:.2f}>{app_config.MAX_QUEUE_WORK_LIFE_TIME}) timed out. "
...
class WorkerManager:
    ...
    def _check_workers(self):
        ...
        for i, worker in enumerate(self.workers):
            if not worker.is_alive():
                ...
                try:
                    ...
                    max_process_time = self.shared.get(
                        worker.worker_id, app_config.MAX_PROCESS_TIME
                    )
                    is_busy = 0
                    is_hanged = 0
                    for subp in worker_p.children(recursive=True):
                        ...
                        if (
                            subp.is_running()
                            and time() - subp.create_time() > max_process_time
                        ):
                            is_hanged = 1
                            logger.info(
                                f"Worker {worker.worker_id} is running for {time() - subp.create_time()} seconds. Terminating..."
                            )
        ...
        logger.info(
            f"Total: {len(self.workers)}, free: {len(self.workers) - busy_workers} failed: {failed_workers}, busy: {busy_workers}, hanged: {hanged_workers}"
        )
