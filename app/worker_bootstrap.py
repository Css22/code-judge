...
def bootstrap_workers_from_yaml(yaml_path: str, state_file: str='.state/state.json') -> None:
    ...
    for name, node in tools.items():
        ...
        if applied_sig == sig:
            ...
            logging.info(f"{LOG_PREFIX} tool {name} has already been initialized and configuration is unchanged, skipping.")
            continue

        logging.info(f"{LOG_PREFIX} Initialize executor {name} …")
        for step in steps:
